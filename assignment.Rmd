...
# 3. Swirl course on ggplot2

In the `swirl()` course menu, select **Exploratory Data Analysis**, complete the lesson **8: GGPlot2 Part2 ** (yes, part2, part1 is more about `qplot` which is not as powerful as `ggplot`). In part2, you can ignore the first question about `qplot` (the answer is `qplot(displ, hwy, data = mpg, geom = c("point", "smooth"), facets = .~drv)`). It is much more versatile to learn and use the full `ggplot` instead. Also one questions requests using a base plot function (which we have not covered and will omit in favor of ggplot) - the answer to this question is `plot(myx, myy, type = "l", ylim = c(-3,3))`. 
...
## 3.1 Now let's use the full power of ggplot to examine the `mpg` dataset

> Reproduce the last plot generated in the swirl exercise. Remember that you'll first have to define the variable `g` in order to use it to build your plot (or build it completely from scratch without the `g` intermediate), and make sure to use proper space around infix operators (` = `, ` ~ `, etc.). Once again, store the final plot in a variable (`p3_1`).

```{r, fig.width=10, fig.height=8}
# store the plot in variable p3_1
p3_1 <- 
...
## 4.1 What’s gone wrong with this code? Why are the points not blue?
...
expect_true(
  exists("p2_1"),
  info = msg("2.1", "'p2_1' plot object should exist but does not")
)

expect_true(
  exists("p3_1"),
  info = msg("3.1", "'p3_1' plot object should exist but does not")
)

expect_true(
  exists("p4_2"),
  info = msg("4.2", "'p4_2' plot object should exist but does not")
)

expect_true(
  exists("p4_4"),
  info = msg("4.4", "'p4_4' plot object should exist but does not")
)

expect_true(
  exists("p4_6"),
  info = msg("4.6", "'p4_6' plot object should exist but does not")
)

expect_true(
  exists("p4_7"),
  info = msg("4.7", "'p4_7' plot object should exist but does not")
)

expect_true(
  exists("p5_1"),
  info = msg("5.1", "'p5_1' plot object should exist but does not")
)

expect_true(
  exists("p5_2"),
  info = msg("5.2", "'p5_2' plot object should exist but does not")
)

expect_true(
  exists("p5_4"),
  info = msg("5.4", "'p5_4' plot object should exist but does not")
)

expect_true(
  exists("p6_2"),
  info = msg("6.2", "'p6_2' plot object should exist but does not")
)
```
